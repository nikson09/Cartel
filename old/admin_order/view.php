<?php include ROOT . '/Site/layouts/header_admin.php'; ?>

<section>
<div class="Register_shop container">
    <div class="container">
        <div class="row">

            <br/>

            <div class="breadcrumbs">
                <ol class="breadcrumb">
                    <li><a href="/admin">Админпанель</a></li>
                    <div class="divider_n"></div>
                    <li><a href="/admin/order">Управление заказами</a></li>
                    <div class="divider_n"></div>
                    <li class="active">Просмотр заказа</li>
                </ol>
            </div>
 </div>
 <br/>
            <h4 class="text-center">Просмотр заказа #<?php echo $order['id']; ?></h4>
           


<br/>


            
<div class="row justify-content-center">

            <h5 class="text-center">Информация о заказе</h5>
            <table class="table-admin-small table-bordered table-striped table">
                <tr>
                    <td>Номер заказа</td>
                    <td><?php echo $order['id']; ?></td>
                </tr>
                <tr>
                    <td>Имя клиента</td>
                    <td><?php echo $order['user_name']; ?></td>
                </tr>
                <tr>
                     <tr>
                    <td>Телефон клиента</td>
                    <td><?php echo $order['user_phone']; ?></td>
                </tr>
                <tr>
                <tr>
                    <td>Выбор связи</td>
                    <td><?php echo $order['user_connection']; ?></td>
                </tr>
                <tr>
                     <tr>
                    <td>Область</td>
                    <td><?php echo $order['user_oblast']; ?></td>
                </tr>
                <tr>
                     <tr>
                    <td>Город</td>
                    <td><?php echo $order['user_city']; ?></td>
                </tr>
            
                    <td>Комментарий клиента</td>
                    <td><?php echo $order['user_comment']; ?></td>
                </tr>
                <?php if ($order['user_id'] != 0): ?>
                    <tr>
                        <td>ID клиента</td>
                        <td><?php echo $order['user_id']; ?></td>
                    </tr>
                <?php endif; ?>
                <tr>
                    <td><b>Статус заказа</b></td>
                    <td><?php echo Order::getStatusText($order['status']); ?></td>
                </tr>
                <tr>
                    <td><b>Дата заказа</b></td>
                    <td><?php echo $order['date']; ?></td>
                </tr>
            </table>

            <h5>Товары в заказе</h5>

            <table class="table-admin-medium table-bordered table-striped table ">
                <tr>
                    <th>Изображение товара</th>
                    <th>ID товара</th>
                    <th>Артикул товара</th>
                    <th>Название</th>
                    <th>Цена</th>
                    <th>Количество</th>
                </tr>
                <?php foreach ($products as $product): ?>
                    <tr>
                        <td><div class="row justify-content-center">
                         <img  src="<?php echo Product::getImage($product['id']); ?>" width="auto" height="149,917" alt="" /></div></td>
                        <td><?php echo $product['id']; ?></td>
                        <td><?php echo $product['code']; ?></td>
                        <td><?php echo $product['name']; ?></td>
                        <td><?php echo $product['price']; ?>грн</td>
                        <td><?php echo $productsQuantity[$product['id']]; ?></td>
                    </tr>
                <?php endforeach; ?>
                <tr>
                                    <td colspan="3">Общая стоимость:</td>
                                    <td>
                                        <div class="row justify-content-center">
                                        <h4><?php echo $totalPrice;?></h4><h6>грн</h6>
                                        </div>
                                        </td>
                                </tr>
            </table>

            <a href="/admin/order/" class="btn btn-default back"><i class="fa fa-arrow-left"></i> Назад</a>
        </div>
</div>
</div>
</section>

<?php include ROOT . '/Site/layouts/footer_admin.php'; ?>